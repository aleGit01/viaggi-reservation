 ...
 * `127.0.0.1:9092` by default.
 *
 * Requires a running ZooKeeper instance to connect to. By default, it expects a
 * ZooKeeper instance running at `127.0.0.1:2181`. You can specify a different
 * ZooKeeper instance by setting the `zookeeper.connect` parameter in the
...
public class KafkaEmbedded {
	 ...
	 *
	 * @param config
	 *            Broker configuration settings. Used to modify, for example, on
	 *            which port the broker should listen to. Note that you cannot
	 ...
	 *
	 * You can use this to tell Kafka producers and consumers how to connect to this
	 * instance.
	 */
	...
	public void createTopic(String topic, int partitions, int replication, Properties topicConfig) {
		...
		KafkaZkClient zkClient = KafkaZkClient.apply(zookeeperConnect(), isSecure, DEFAULT_ZK_SESSION_TIMEOUT_MS,
				DEFAULT_ZK_CONNECTION_TIMEOUT_MS, maxInFlightRequests, time, metricGroup, metricType);
