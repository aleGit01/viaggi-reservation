package com.ale.viaggi.reservation.test.embedded.kafka;
...
/**
 * Runs an in-memory, "embedded" Kafka cluster with 1 ZooKeeper instance, 1
 * Kafka broker, and 1 Confluent Schema Registry instance.
 */
public class EmbeddedSingleNodeKafkaCluster extends ExternalResource {
	...
	public void start() throws Exception {
		...
		zkClient = KafkaZkClient.apply(zookeeper.connectString(), JaasUtils.isZkSecurityEnabled(), 30000, 30000,
	 ...
	 *
	 * You can use this to tell Kafka Streams applications, Kafka producers, and
	 * Kafka consumers (new consumer API) how to connect to this cluster.
	 */
	...
	/**
	 * This cluster's ZK connection string aka `zookeeper.connect` in
	 * `hostnameOrIp:port` format. Example: `127.0.0.1:2181`.
	 *
	 * You can use this to e.g. tell Kafka consumers (old consumer API) how to
	 ...
	 *
	 * @param timeoutMs
	 *            the max time to wait for the topics to be deleted (does not block
	 *            if {@code <= 0})
	 * @param topics
	...
	private final class TopicsDeletedCondition implements TestCondition {
		...
		public boolean conditionMet() {
			// final Set<String> allTopics = new
			// HashSet<>(scala.collection.JavaConversions.seqAsJavaList(zkUtils.getAllTopics()));

			final Set<String> allTopics = new HashSet<>(JavaConverters.seqAsJavaList(zkClient.getAllTopicsInCluster()));
